...
export default function Header() {
  ...
  return (
    <header className="border-b border-[#2A2A3A] bg-[#0A0A0F]/80 backdrop-blur-xl sticky top-0 z-50">
      <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
      ...
      {/* Mobile Menu */}
      {showMobileMenu && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-[#0A0A0F] border-b border-[#2A2A3A] shadow-lg">
          <nav className="flex flex-col p-4 gap-2">
            ...
            <Link
              href="/credits"
              className="px-4 py-3 rounded-lg text-sm text-[#A0A0B0] hover:bg-[#151520] hover:text-white transition-colors flex items-center justify-between"
              style={{ fontFamily: 'Space Grotesk, sans-serif' }}
              onClick={() => setShowMobileMenu(false)}
            >
              <span>积分充值</span>
