...
BEGIN
    IF EXISTS (
        ...
        CHECK (transaction_type IN ('purchase', 'deduct', 'refund'));
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        SELECT 1 FROM pg_constraint
        WHERE conname = 'generation_tasks_status_check'
    ) THEN
...
-- 8. 为现有的 auth.users 创建 public.users 记录
